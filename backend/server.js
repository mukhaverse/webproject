...
function getHighestSeverity(results) {
  let highest = { severity: "none", rank: 0 };
...
function getDrugColors(count) {
  const colors = ["pink", "yellow", "blue", "orange", "green", "purple", "cyan"];
  return colors.slice(0, count);
}

function getScheduleRule(rank) {
  if (rank === 1) return { canSchedule: true, gapHours: 2, message: "Spacing medications by 1–4 hours is usually enough because the issue is often absorption-based." };
  if (rank === 2) return { canSchedule: true, gapHours: 4, message: "Spacing medications by 2–4 hours may help when the issue is absorption or binding-related." };
  if (rank === 3) return { canSchedule: false, gapHours: null, message: "No fixed time interval can reliably make this combination safe. Timing alone does not resolve this interaction." };
  if (rank === 4) return { canSchedule: false, gapHours: null, message: "No safe interval exists. This medication combination should be avoided entirely." };
  return { canSchedule: false, gapHours: null, message: "No schedule recommendation available." };
}

function buildScheduleRecommendation(normalizedDrugs, results) {
  const highest = getHighestSeverity(results);

  if (highest.rank === 0) return { show: false };

  const rule = getScheduleRule(highest.rank);

  if (!rule.canSchedule) {
    return { show: true, canSchedule: false, severity: highest.severity, message: rule.message };
  }
  ...
  const scheduleData = normalizedDrugs.map((drug, index) => ({
    drug: drug.normalized,
    time: formatHour(startHour + index * rule.gapHours),
    color: colors[index] || "green"
  }));
...
app.post("/normalize", async (req, res) => {
  ...
  try {
    ...
    for (let drug of drugs) {
      const normalized = await normalizeDrug(drug);
      results.push({ original: drug, normalized });
    }
...
app.post("/testimonials", (req, res) => {
  ...
  db.query("INSERT INTO testimonials (name, message) VALUES (?, ?)", [name, message], (err) => {
    if (err) {
      console.error("DB ERROR:", err);
      return res.status(500).json({ error: "Database error" });
    }
...
io.on("connection", (socket) => {
  ...
  socket.on("send-message", (data) => {
    if (!data || !data.chatId || !data.body?.trim() || !data.senderRole) return;
    io.to(String(data.chatId)).emit("receive-message", data);
    io.to("admins").emit("conversation-updated");
  });
